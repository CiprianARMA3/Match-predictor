...
def get_competition_matches(comp_code: str, offset=0, limit=10):
...
def get_team_stats(team_id: int):
    ...
    if not matches:
        return {"gf_home":1,"ga_home":1,"gf_away":1,"ga_away":1}
...
# -------------------- HEAD TO HEAD --------------------
def get_head_to_head(home_id, away_id, matches=20):
    ...
    for m in data.get("matches",[]):
        if {m["homeTeam"]["id"], m["awayTeam"]["id"]} == {home_id, away_id}:
...
def update_elo(home_id, away_id, home_goals, away_goals):
    ...
    multiplier = 1 + (diff-1)*0.1 if diff>1 else 1
    if home_goals > away_goals:
        ...
        s_h, s_a = 0,1
    home_elo += K_FACTOR*multiplier*(s_h - expected_home)
    away_elo += K_FACTOR*multiplier*(s_a - expected_away)
    set_elo(home_id, home_elo)
    set_elo(away_id, away_elo)

# -------------------- MONTE CARLO --------------------
def monte_carlo(lambda_home, lambda_away, sims=MC_SIMS):
...
def predict(home_stats, away_stats, home_id, away_id, home_adv=1.1):
    ...
    home_elo = get_elo(home_id) + HOME_ADV_ELO
    away_elo = get_elo(away_id)
    elo_diff = home_elo - away_elo
    factor = 10 ** (elo_diff/400)
    lambda_home *= factor/(factor+1)
    lambda_away *= 1/(factor+1)
...
def estimate_corners(home_id, away_id):
    ...
    h_c = h_stats["gf_home"]*3 + h_stats["ga_home"]*2
    a_c = a_stats["gf_away"]*3 + a_stats["ga_away"]*2
